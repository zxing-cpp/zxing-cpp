...
TEST(MQRDecoderTest, VersionM4WithVersionM3Encoded)
{
	const auto bitMatrix = ParseBitMatrix("XXXXXXX X X X X  \n"
										  "X     X XX X   XX\n"
										  "X XXX X  X  X  XX\n"
										  ...
										  88, false);
	const auto result = Decode(bitMatrix, {}, true);
	EXPECT_EQ(DecodeStatus::FormatError, result.errorCode());
}

TEST(MQRDecoderTest, VersionM1WithDifferentVersionsEncoded)
{
	const auto bitMatrix = ParseBitMatrix("XXXXXXX X  \n"
										  "X     X    \n"
										  "X XXX X XXX\n"
										  "X XXX X  XX\n"
										  "X XXX X   X\n"
