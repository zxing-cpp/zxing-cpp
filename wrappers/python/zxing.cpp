...
BarcodeFormats BarcodeFormatsFromPy(nb::object formats)
{
	if (formats.is_none())
		return {};
	if (nb::isinstance<BarcodeFormat>(formats))
		return {nb::cast<BarcodeFormat>(formats)};
	if (nb::isinstance<BarcodeFormats>(formats))
		return nb::cast<BarcodeFormats>(formats);
	if (nb::isinstance<nb::typed<nb::list, BarcodeFormat>>(formats)) {
		return BarcodeFormats(nb::cast<std::vector<BarcodeFormat>>(formats));
	}
	nb::raise_type_error("Invalid formats type");
	return {};
}
...
std::optional<Barcode> read_barcode(nb::object _image, nb::object formats, bool try_rotate, bool try_downscale,
									TextMode text_mode, Binarizer binarizer, bool is_pure, EanAddOnSymbol ean_add_on_symbol,
									bool return_errors)
{
	auto res = read_barcodes_impl(_image, BarcodeFormatsFromPy(formats), try_rotate, try_downscale, text_mode, binarizer, is_pure, ean_add_on_symbol,
								  return_errors, 1);
	return res.empty() ? std::nullopt : std::optional(res.front());
}

Barcodes read_barcodes(nb::object _image, nb::object formats, bool try_rotate, bool try_downscale, TextMode text_mode,
					   Binarizer binarizer, bool is_pure, EanAddOnSymbol ean_add_on_symbol, bool return_errors)
{
	return read_barcodes_impl(_image, BarcodeFormatsFromPy(formats), try_rotate, try_downscale, text_mode, binarizer, is_pure, ean_add_on_symbol,
							  return_errors);
}
...
Image write_barcode(BarcodeFormat format, nb::object content, int width, int height, int quiet_zone, int ec_level)
{
	...
	BitMatrix bits;
	if (nb::str content_str; nb::try_cast(content, content_str))
		bits = writer.encode(nb::cast<std::string>(content_str), width, height);
	else if (nb::bytes content_bytes; nb::try_cast(content, content_bytes))
...
#undef X
	...
	pyBarcodeFormats
		...
		.def("__eq__", [](const BarcodeFormats& f1, nb::object f2) {
			try {
				return f1 == BarcodeFormatsFromPy(f2);
	...
	nb::implicitly_convertible<nb::iterable, BarcodeFormats>();
