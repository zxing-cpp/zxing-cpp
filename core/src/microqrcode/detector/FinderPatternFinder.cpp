...
FinderPatternFinder::FinderPatternFinder(const BitMatrix& image) :
	crossCheckStateCount_(5)
{
	image_ = image.copy();
}
...
FinderPatternInfo FinderPatternFinder::generatePatternInfoForPattern(const FinderPattern& actualPattern)
{
	...
	return FinderPatternInfo(std::vector<FinderPattern>{
	  actualPattern,
	  fakeTopRightPattern,
	  fakeBottomLeftPattern});
}
...
bool FinderPatternFinder::foundPatternCross(const std::vector<int>& stateCount) const
{
	...
	return std::abs(moduleSize - stateCount[0]) < maxVariance && abs(moduleSize - stateCount[1]) < maxVariance && abs(3.0f
...
bool FinderPatternFinder::crossCheckDiagonal(int startI, int centerJ, int maxCount, int originalStateCountTotal) 
{
	...
	while (startI + i < maxI && centerJ + i < maxJ && !image_.get(centerJ + i, startI + i) && stateCount[3] < maxCount) 
	{
		stateCount[3]++;
		i++;
	}
	...
	int stateCountTotal = stateCount[0] +
		...
		stateCount[3] +
		stateCount[4];
	return std::abs(stateCountTotal - originalStateCountTotal) < 2 * originalStateCountTotal && foundPatternCross(stateCount);
}
...
bool FinderPatternFinder::handlePossibleCenter(const std::vector<int>& stateCount, int i, int j, bool pureBarcode) 
{
	...
	if (!isnan(centerI)) 
	{
		...
		if (!isnan(centerJ) && (!pureBarcode || crossCheckDiagonal(std::lroundf(centerI), std::lroundf(centerJ), stateCount[2], stateCountTotal)))
		{
