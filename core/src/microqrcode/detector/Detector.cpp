...
DetectorResult Detector::detect(DecodeHints const& hints) const
{
	...
	int dimensionFromCenters = computeDimension(patternInfo.getActualTopLeft(), patternInfo.getFakeTopRight(), patternInfo.getFakeBottomLeft(), moduleSize);
...
DetectorResult Detector::detectPure() const
{
	...
	return { std::move(bits), {{borderWidth, borderWidth}, {rowEndOfSymbol, borderWidth}, {rowEndOfSymbol, rowEndOfSymbol}, {borderWidth, rowEndOfSymbol}} };
}

DetectorResult Detector::processCodeEnclosingRect(const std::vector<ResultPoint>& codeEnclosingRect, int dimension) const
{
...
{
	const QuadrilateralF codeDomain = {
		PointF{0.0f, 0.0f},
		PointF{static_cast<float>(dimension), 0.0f},
		PointF{static_cast<float>(dimension), static_cast<float>(dimension)},
		PointF{0.0f, static_cast<float>(dimension)}
	};
	const QuadrilateralF imageDomain = {
		rect[0],
		rect[2],
		rect[3],
		rect[1]
...
* @return Transform from registered QR code to input image.
*/
PerspectiveTransform Detector::createTransform(const ResultPoint& topLeft, const ResultPoint& topRight, const ResultPoint& bottomLeft, int dimension) const
{
	...
	const QuadrilateralF codeDomain = {
		PointF{PATTERN_CENTER_POS, PATTERN_CENTER_POS},
		PointF{dimMinusThree, PATTERN_CENTER_POS},
		PointF{dimMinusThree, dimMinusThree},
...
/**
  ...
  */
int Detector::computeDimension(const ResultPoint& topLeft, const ResultPoint& topRight, const ResultPoint& bottomLeft, float moduleSize) const
{
	float tltrCentersDimension = ResultPoint::Distance(std::lround(topLeft.x()), std::lround(topLeft.y()), std::lround(topRight.x()), std::lround(topRight.y())) / moduleSize;
	float tlblCentersDimension = ResultPoint::Distance(std::lround(topLeft.x()), std::lround(topLeft.y()), std::lround(bottomLeft.x()), std::lround(bottomLeft.y())) / moduleSize;
	// 7 is added to the dimension because the centers of each pattern should be at (3.5, 3.5) away from the nearest corner of the code.
...
{
	float tltrDimension = ResultPoint::Distance(std::lround(codeEnclosingRect[0].x()), std::lround(codeEnclosingRect[0].y()), std::lround(codeEnclosingRect[2].x()), std::lround(codeEnclosingRect[2].y())) / moduleSize;
	float tlblDimension = ResultPoint::Distance(std::lround(codeEnclosingRect[0].x()), std::lround(codeEnclosingRect[0].y()), std::lround(codeEnclosingRect[1].x()), std::lround(codeEnclosingRect[1].y())) / moduleSize;
